...
print(x.grad)
'''
感觉这里涉及torch自身的反向传播机制的设计，如果只是使用参数本身，那么在这个sgd参数更新其实可能也会变成网络结构的一部分，
这时候网络就不是线性回归了，而使用.data可能就是隔断了梯度传导，让这里只是一个参数数值更新

我赞同你的理解，根据代码，param就是模型参数[w和b]，在损失函数进行求梯度时，
