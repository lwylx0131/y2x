...
'''
词是表义的基本单元，词向量是用来表示词的向量，也可被认为是词的特征向量或表征，把词映射为实数域向量的技术叫词嵌入(word embedding)。
虽然one-hot词向量构造起来很容易，但通常并不是一个好选择。原因：one-hot词向量无法准确表达不同词之间的相似度，如余弦相似度。
由于任何两个不同的one-hot向量的余弦相似度都为0，很多不同词之间的相似度难以通过one-hot向量准确的体现出来。
而word2vec工具的提出正是为了解决上面的问题，它将每个词表示成一个定长的向量，并使得这些向量能较好的表达不同词之间的相似和类比关系。
...
#在这里我们主要介绍Skip-Gram模型的实现，CBOW实现与其类似。
'''
简单来说，Word2Vec能从语料中学到如何将离散的词映射为连续空间中的向量，并保留其语义上的相似关系。
那么为了训练Word2Vec模型，我们就需要一个自然语言语料库，模型将从中学习各个单词间的关系，这里我们使用经典的PTB语料库进行训练。
...
with open('./ptb/ptb.train.txt', 'r') as f:
    ...
    raw_dataset = [st.split() for st in lines] # st是sentence缩写，单词以空格为分割
...
# Skip-Gram跳字模型，PyTorch预置的Embedding层
