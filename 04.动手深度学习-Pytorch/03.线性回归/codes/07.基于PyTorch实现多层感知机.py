...
'''
K折交叉验证：
由于验证数据集不参与模型训练，当训练数据不够用时，预留大量的验证数据显得太奢侈。一个改善的方式是K折交叉验证（K-fold cross-validation）。
在K折交叉验证中，我们把原始训练数据集分割称K个不重合的子数据集，然后我们做K次模型训练和验证。
每一次，我们使用一个子数据集验证模型，使用其他K-1个子数据集来训练模型。
在这个K此训练和验证中，每次用来验证模型的子数据集都不同。
最后对这K次训练误差和验证误差分别求平均。
'''
